  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () async {
          if (gp.nosSelected <= 0) {
            Toast.show("No members selected to add!", context);
            return;
          }
          showProgress();
          bool f = await groupProvider.addMember(widget.gm);
          if (f) {
            Toast.show("Members Added", context);
            Navigator.of(context, rootNavigator: true).pop();
            Navigator.of(context).pop();
          } else {
            Toast.show("Failed to add members", context);
            Navigator.of(context, rootNavigator: true).pop();
          }
        },
      ),
        ...
        ? Container(
            ...
            child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  for (Members m in widget.gm.members) {
                    if (m.id == myContacts[index].cid) {
                      alreadyExist = true;
                    }
                  }
                  return GestureDetector(
                    onTap: () {
                      if (alreadyExist) {
                        Toast.show("Already member", context);
                        return;
                      }
                      // do selection
